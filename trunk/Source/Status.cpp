...
kAway2::Status::Status(const int _cfgCol)
{
  this->cfgCol = _cfgCol;
}

kAway2::Status::~Status()
{
  this->SaveNets();
  this->info.clear();
  this->nets.clear();
...
void kAway2::Status::ChangeStatus( const char *txt, int st )
{
...
void kAway2::Status::ChangeStatus(int net, const char *txt, int st )
{
  std::string buff, status(txt);
...
std::string kAway2::Status::LimitChars( std::string Status, int net, int s )
{
  UINT iLimit;
...
void kAway2::Status::Init()
{
  this->GetNets();
}

int kAway2::Status::GetNets(bool mask)
{
  int id, type, net, a=0;
  char *txt;
...
bool kAway2::Status::NetState( int net )
{
  itemNet v;
  for( std::list<itemNet>::iterator it = this->nets.begin(); it != this->nets.end(); it++ )
  {
...
bool kAway2::Status::SetNet( int net, bool use )
{
  itemNet v;
...
void kAway2::Status::AddInfo( char *info, int net )
{
  itemInfo in;
  std::string txt(info);
  bool t = false;
...
void kAway2::Status::RememberInfo()
{
  char *st = 0;
  itemNet item;
...
void kAway2::Status::RememberInfo( int net )
{
  char *st = NULL;
...
std::string kAway2::Status::GetInfo( int net )
{
  std::string t("");
  itemInfo v;
...
void kAway2::Status::BackInfo( int net )
{
  std::string status;
...
void kAway2::Status::BackRemeberedInfo()
{
  itemInfo v;
  for( std::list<itemInfo>::iterator it = this->info.begin(); it != this->info.end(); it++ )
  {
...
bool kAway2::Status::IsIgnored( int net )
{
  bool is = false;
...
std::string kAway2::CtrlStatus::Format( std::string txt, int net )
{
