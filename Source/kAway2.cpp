...
namespace kAway2 {
  ...
  int IEnd() {
    ...
    delete fCtrl; fCtrl = NULL;
    delete sCtrl; sCtrl = NULL;
    delete wCtrl; wCtrl = NULL;
    delete MRUlist; MRUlist = NULL;
  ...
  int IPrepare() {
    ...
    sCtrl = new FormattedStatus(lCtrl::status, cfg::status::whenInvisible, cfg::status::dotsAppend, "status");
    ...
    log("[kAway2::IPrepare()]: Ctrl = %i, pCtrl = %i, fCtrl = %i, sCtrl = %i, wCtrl = %i", 
    ...
    */
    ...
    sCtrl->oFormat->UIDrawHelpBtn(stVars, ui::statusCfgGroup);
    ...
    */
    ...
    sCtrl->oFormat->UIDrawHelpBtn(rVars, ui::replyCfgGroup);
    ...
    sCtrl->oFormat->UIDrawHelpBtn(rVars, ui::cntCfgGroup);
    ...
    if (GETINT(cfg::btnInMainWindow)) {
      UIActionAdd(pCtrl->getPluginsGroup(), ui::powerInMainWnd, 0, "W³¹cz tryb away", ico::enable);
    }
  ...
  int actionProc(sUIActionNotify_base *anBase) {
    ...
    if (fCtrl) {
      fCtrl->onAction(anBase->act.id, an->code);
    }
  ...
  switch (msgBase->id) {
    ...
    case IM_MSG_RCV: {
      ...
      if (userMsg && (m->body[0] == '/' && strlen(m->body) > 1) && Helpers::altCfgVal(cnt, cfg::ircCmds)) {
        ...
        if (cmd == "away") {
          del = true; set = pCtrl->enable(awayMsg, st);
        } else if (cmd == "brb") {
          del = true; set = pCtrl->enable(awayMsg, st, true);
        } else if (cmd == "back") {
          del = true; set = pCtrl->disable(awayMsg);
        } else if (cmd == "re") {
          del = true; set = pCtrl->disable(awayMsg, true);
        }
    ...
    case api::showAwayWnd: {
      logDebug("Remote API Call [showAwayWnd]: from = %s",
        Helpers::getPlugName(msg->sender));
      wCtrl->show();
      break;
    }
