  ...
  public:
    ...
    void inline onPluginsLoaded(Controller* self) {
      if (int oldId = Helpers::pluginExists(plugsNET::kaway)) {
        Ctrl->IMessage(&sIMessage_plugOut(oldId, "kAway2 jest następcą wtyczki K.Away :)",
      ...
      if (int ggCrypt = Helpers::pluginExists(plugsNET::ggcrypt)) {
        Ctrl->IMessage(&sIMessage_plugOut(ggCrypt, "Wtyczka GG Crypt jest przestarzała, przy czym\n"
          "nie pozwala na poprawne działanie wtyczki kAway2.",
          sIMessage_plugOut::erNo, sIMessage_plugOut::euNowAndOnNextStart));
        return this->setFailure();
      }
      this->setSuccess();
