...
kAway::Control *kAway::Control::sCtrl = NULL;
UINT_PTR kAway::Control::m_Timer = NULL;

kAway::Control::Control() {
	this->ctrlSt = new kAway::CtrlStatus(kAway::cfg::netChange);
	this->ctrlSt->Init();
...
void kAway::Control::Enable(std::string msg) {
  if (!this->isOn) {
...
void kAway::Control::Disable(std::string msg) {
  if (this->isOn) {
...
void kAway::Control::netListInit(int colCount, int cfgGroup, int dynActGroup) {
	itemNet item;
	int c = 1, i = 0, ico;
...
void kAway::Control::netListSetState(int cfgGroup, int dynActGroup) {
	itemNet item;
...
void kAway::Control::netListGetState(int cfgGroup, int dynActGroup) {
	itemNet item;
	char buff[16];
	int v;
